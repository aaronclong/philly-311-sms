...
class Response {
	...
	* defRequest() {
	...
	setResponse() {
		this.response = "Sorry there was some kind of error please try again";
		let { Body } = this.meta;
		if (this.claim.state === 3){
			if (Body == "311" || Body == "00") {
				console.log(this.claim.state + "; ATT 3!! ");
				this.response = rspOne[Body];
				this.addToClaim({ state: 3, messages: this.request });
			} else if (rspOne[Body] !== undefined ) {
				console.log(this.claim.state + "; ATT Undefined");
				this.response = rspOne[Body];
			...
			console.log("1; At Local");
			this.response = "Thank you, your claim number is " + this.claim.id;
